...
func (r *Resolver) FetchRandomOne(address string) (net.IP, error) {
	ips, err := r.Fetch(address)
	if err != nil || len(ips) == 0 {
		return nil, err
	}
...
func (r *Resolver) FetchRandomOneString(address string) (string, error) {
	ip, err := r.FetchRandomOne(address)
	if err != nil || ip == nil {
		return "", err
	}
	return ip.String(), nil
