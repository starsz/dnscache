...
func TestFetchOneLoadsTheRandomValue(t *testing.T) {
	r := New(0)
	r.cache["something.viki.io"] = []net.IP{net.ParseIP("100.100.102.103"), net.ParseIP("100.100.102.104")}
	ip, _ := r.FetchRandomOne("something.viki.io")
	assertIps(t, []net.IP{ip}, []string{"100.100.102.103", "100.100.102.104"})
}

func TestFetchOneStringLoadsTheRandomValue(t *testing.T) {
	r := New(0)
	r.cache["something.viki.io"] = []net.IP{net.ParseIP("100.100.102.103"), net.ParseIP("100.100.102.104")}
	ip, _ := r.FetchRandomOneString("something.viki.io")
	if ip != "100.100.102.103" && ip != "100.100.102.104" {
		t.Errorf("expected 100.100.102.103 or 100.100.102.104 but got %v", ip)
	}
}
